    ...
    <header className="fixed top-0 left-3 right-3 z-50 bg-white/95 backdrop-blur-sm shadow-sm my-2 rounded-xl border border-gray-100">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3 md:px-6">
        ...
        <Link href="/" className="flex items-center space-x-2 group">
          ...
          />
          <span className="md:text-2xl text-base font-bold bg-gradient-to-r from-orange-500 to-amber-500 bg-clip-text text-transparent">
        ...
        <nav className="hidden md:flex items-center space-x-1">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-gray-600 hover:text-white hover:bg-blue-500 rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200"
            >
              {item.name}
            </Link>
          ))}
        ...
        <nav className="flex flex-col px-4 pb-4 pt-1 space-y-1 border-t border-gray-100">
          {navigation.map((item) => (
            <Link
              ...
              className="text-gray-600 hover:text-gray-900 hover:bg-blue-500 rounded-lg px-4 py-2.5 text-sm font-medium transition-all duration-200"
            >
              {item.name}
            </Link>
          ))}
